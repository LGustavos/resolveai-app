...
export default function RegisterPage() {
  ...
  const providerSignup = searchParams.get("role") === "provider";
  ...
  useEffect(() => {
    if (providerSignup && role !== "PROVIDER") {
      setRole("PROVIDER");
      setStep(1);
    }
  }, [providerSignup, role]);
    ...
    <div>
      <div className="mb-6 flex flex-col items-center text-center">
        <h1 className="text-2xl font-bold tracking-tight">
          {providerSignup ? "Criar perfil de prestador" : "Criar conta"}
        </h1>
        <p className="mt-1 text-sm text-muted-foreground">
          {step === 1
            ? providerSignup
              ? "Cadastre seu servico e comece a receber contatos."
          ...
          >
            ...
            <div className="flex gap-3">
              ...
              <Button type="submit" className="flex-1 h-11 rounded-lg font-semibold gradient-bg" disabled={loading}>
                {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : providerSignup ? "Criar perfil" : "Criar conta"}
              </Button>
            </div>
          </form>
        )}
